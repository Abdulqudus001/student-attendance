...
(function (global, factory) {
  ...
  const Zepto = (function () {
    ...
    const isArray = Array.isArray ||
        function (object) { return Array.isArray(object) }
    ...
    zepto.fragment = function (html, name, properties) {
      ...
      if (isPlainObject(properties)) {
        ...
        $.each(properties, function (key, value) {
          if (methodAttributes.includes(key)) { nodes[key](value) } else { nodes.attr(key, value) }
        })
      }
    ...
    zepto.qsa = function (element, selector) {
      ...
      const nameOnly = maybeID || maybeClass ? selector.slice(1) : selector // Ensure that a 1 char tag name still gets checked
    ...
    $.fn = {
      ...
      not (selector) {
        ...
        if (isFunction(selector) && selector.call !== undefined) {
          ...
          this.forEach(function (el) {
            if (!excludes.includes(el)) { nodes.push(el) }
          })
        }
        return $(nodes)
      },
      ...
      closest (selector, context) {
        ...
        this.each(function (_, node) {
          while (node && !(collection ? collection.includes(node) : zepto.matches(node, selector))) { node = node !== context && !isDocument(node) && node.parentNode }
      ...
      parents (selector) {
        ...
        while (nodes.length > 0) {
          nodes = $.map(nodes, function (node) {
            if ((node = node.parentNode) && !isDocument(node) && !ancestors.includes(node)) {
              ancestors.push(node)
              return node
            }
          })
  ...
  ; (function ($) {
    ...
    function ajaxSuccess (data, xhr, settings, deferred) {
      const context = settings.context; const status = 'success'
    ...
    $.ajaxJSONP = function (options, deferred) {
      ...
      const xhr = { abort }; let abortTimeout
    ...
    $.ajax = function (options) {
      ...
      let dataType = settings.dataType; const hasPlaceholder = /\?.+=\?/.test(settings.url)
      ...
      if (mime = settings.mimeType || mime) {
        if (mime.includes(',')) { mime = mime.split(',', 2)[0] }
    ...
    function serialize (params, obj, traditional, scope) {
      let type; const array = $.isArray(obj); const hash = $.isPlainObject(obj)
  ...
  ; (function ($) {
    $.fn.serializeArray = function () {
      let name; let type; const result = []
